...
### Capa 1: Modelos (Models) - `src/finance_app/models/`
...
**`src/finance_app/models/__init__.py`**
```python
...
**`src/finance_app/models/account.py`** - Modelo de cuentas
```python
class Account(Base):
...
**`src/finance_app/models/transaction.py`** - Modelo de transacciones
```python
...
**`src/finance_app/models/budget.py`** - Modelo de presupuestos
```python
...
**`src/finance_app/models/exchange_rate.py`** - Tasas de cambio históricas
```python
...
### Capa 2: Servicios (Services) - `src/finance_app/services/`
...
**`src/finance_app/services/budget_service.py`** - Lógica de presupuesto
```python
...
**`src/finance_app/services/exchange_rate_service.py`** - Tasas de cambio
```python
...
**`src/finance_app/services/transaction_service.py`** - Transacciones y transferencias
```python
...
### Capa 3: API (Endpoints) - `src/finance_app/api/`
...
**`src/finance_app/api/transactions.py`**
```python
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from pydantic import BaseModel
from finance_app.database import get_db
from finance_app.services.transaction_service import create_transfer, delete_transaction
...
from sqlalchemy.orm import Session
from finance_app.database import get_db
from finance_app.services.budget_service import get_month_budget, update_budget_assignment
...
**`src/finance_app/app.py`** - Aplicación principal
```python
...
app.mount("/static", StaticFiles(directory="src/finance_app/static"), name="static")

# Templates
templates = Jinja2Templates(directory="src/finance_app/templates")
...
```html
<!-- src/finance_app/templates/accounts.html -->
...
fetch() a /api/endpoint
      ↓
FastAPI endpoint (src/finance_app/api/)
      ↓
Servicio procesa (src/finance_app/services/)
      ↓
Modelo actualiza DB (src/finance_app/models/)
      ↓
Respuesta JSON
      ↓
...
2. JavaScript captura submit
   └─> src/finance_app/templates/transactions.html:
...
3. Request llega a FastAPI
   └─> src/finance_app/app.py routing a transactions.router
...
5. Servicio ejecuta lógica
   └─> src/finance_app/services/transaction_service.py:
...
6. Modelo guarda en DB
   └─> src/finance_app/models/transaction.py (SQLAlchemy ORM)
...
**1. Crear archivo en `src/finance_app/models/`**

```python
# src/finance_app/models/goal.py
...
**2. Exportar en `src/finance_app/models/__init__.py`**

```python
from finance_app.models.goal import Goal
...
```python
# src/finance_app/models/category.py
class Category(Base):
    # ... campos existentes ...
...
**1. Crear archivo en `src/finance_app/api/` o agregar a uno existente**

```python
# src/finance_app/api/goals.py
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from pydantic import BaseModel
from finance_app.database import get_db
from finance_app.models import Goal
...
**2. Registrar router en `src/finance_app/app.py`**

```python
from finance_app.api import goals
...
**1. Crear template en `src/finance_app/templates/`**

```html
<!-- src/finance_app/templates/goals.html -->
{% extends "layout.html" %}
...
**2. Agregar ruta en `src/finance_app/app.py`**
...
**3. Agregar link en navegación (`src/finance_app/templates/layout.html`)**
...
1. **Hipotecas con cuota fija y tasa efectiva anual**
   - Modificar `src/finance_app/services/mortgage_service.py`
   - Actualizar `src/finance_app/models/account.py` para guardar parámetros

2. **Presupuesto sin selector de moneda**
   - Modificar `src/finance_app/templates/budget.html`
...
4. **Rollover behavior manual**
   - Agregar campo `rollover_behavior` a `src/finance_app/models/category.py`
   - Opciones: 'accumulate' (saving) o 'reset' (gasto mensual)

5. **Transacciones automáticas mejoradas**
   - Ya existe `src/finance_app/models/recurring_transaction.py`
...
- [ ] **Modelo**: Crear/modificar en `src/finance_app/models/`
- [ ] **Exportar**: Agregar a `src/finance_app/models/__init__.py`
- [ ] **Migración**: Borrar DB y ejecutar `python src/finance_app/scripts/init_db.py`
- [ ] **Servicio**: Crear lógica en `src/finance_app/services/`
- [ ] **API**: Crear endpoint en `src/finance_app/api/`
- [ ] **Router**: Registrar en `src/finance_app/app.py`
- [ ] **Template**: Crear HTML en `src/finance_app/templates/`
- [ ] **Ruta HTML**: Agregar en `src/finance_app/app.py`
- [ ] **Navegación**: Actualizar `src/finance_app/templates/layout.html`
...
tree src/finance_app/

# Buscar en código
grep -r "function_name" src/finance_app/
